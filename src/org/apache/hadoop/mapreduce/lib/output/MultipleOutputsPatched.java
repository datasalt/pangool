 ...
 *  * IMPORTANT!!!
 * 
 * Version of this class patched with https://issues.apache.org/jira/browse/MAPREDUCE-2225
 * using as base version cdh3 hadoop-0.20.1+169.88. Once you change the version
 * of Hadoop to one that includes the patch, please, stop to use this
 * classes. 
 * 
 ...
 * counters group is the {@link MultipleOutputsPatched} class name. The names of the 
...
public class MultipleOutputsPatched<KEYOUT, VALUEOUT> {
   ...
   */
  private static final String COUNTERS_GROUP = MultipleOutputsPatched.class.getName();
   ...
   * 
   * The counters group is the {@link MultipleOutputsPatched} class name.
   ...
   */
  public MultipleOutputsPatched(
      TaskInputOutputContext<?, ?, KEYOUT, VALUEOUT> context) {
    this.context = context;
    namedOutputs = Collections.unmodifiableSet(
      new HashSet<String>(MultipleOutputsPatched.getNamedOutputsList(context)));
  ...
  private synchronized RecordWriter getRecordWriter(
    ...
    if (writer == null) {
      ...
      /* !!!!!!!!!!!!!!! CHANGED BY IVAN !!!!!!!!!!!!!!!!!
       * OLD LINE: FileOutputFormat.setOutputName(taskContext, baseFileName);
       * Changed because this method is protected, and cannot be accessed
       * from a differente Jar, so without the patch won't work
       * on production
       */   
