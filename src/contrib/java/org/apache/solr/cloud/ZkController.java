...
public final class ZkController {
  ...
  public void setConfignameToCollection(String configName,String collection) throws IOException, KeeperException, InterruptedException{
      ...
      log.error("Specified collection does not exist in ZooKeeper:" + collection);
      throw new ZooKeeperException(SolrException.ErrorCode.SERVER_ERROR,
          "Specified collection does not exist in ZooKeeper:" + collection);
    }
  	
    ZkNodeProps props = new ZkNodeProps();
    props.put(CONFIGNAME_PROP, configName);
  	String path=ZkStateReader.COLLECTIONS_ZKNODE + "/" + collection;
  	byte[] data = props.store();
