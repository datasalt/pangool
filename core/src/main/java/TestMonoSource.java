...
//		userFields.add(new Field("my_bytes", Schema.create(Type.BYTES),null,null));
		...
		CoGrouperConfig grouperConf = b.build();

		System.out.println(grouperConf.getSchemasBySource());
		SerializationInfo serInfo = SerializationInfo.get(grouperConf);
		System.out.println(serInfo.getIntermediateSchema());
		Path outputPath = new Path("avrool_output");
		CoGrouper coGrouper = new CoGrouper(grouperConf,new Configuration());
