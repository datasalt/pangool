 ...
 */
public class PangoolDriver {
	/**
	 * A description of a program based on its class and a human-readable description.
	 * 
	 * @date april 2006
	 */
	Map<String, ProgramDescription> programs;

	public PangoolDriver() {
  ...
  public void addClass(String name, Class mainClass, String description) throws Throwable {
	...
	public void driver(String[] args) throws Throwable {
		...
		if(args.length == 0) {
			...
			printUsage(programs);
			throw new IllegalArgumentException("An example program must be given " + "as the first argument.");
		}
		...
		if(pgm == null) {
			...
			printUsage(programs);
			throw new IllegalArgumentException("Unknown program '" + args[0] + "' chosen.");
		}
