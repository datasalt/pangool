...
//	private Map<Integer, RawComparator[]> particularInstancedComparators; 
	...
	protected SortBy commonCriteria;
...
//	}
	...
	private void instanceComparators() {
		// TODO add comparators for particular
		SortBy criteria = config.getCommonOrder();
		this.instancedComparators = new RawComparator[criteria.getElements().size()];
		for(int i = 0; i < criteria.getElements().size(); i++) {
			SortElement sortElement = criteria.getElements().get(i);
			Class<? extends RawComparator> clazz = sortElement.getCustomComparator();
			if(clazz != null) {
...
//	}
	...
	public void setConf(Configuration conf) {
		if(conf != null) {
			 ...
			 */
			try {
				config = CoGrouperConfig.get(conf);
				instanceComparators();
				nSchemas = config.getSourceSchemas().values().size();
				SortBy commonSortBy = config.getCommonOrder();
