...
public class TupleTextInputFormat extends FileInputFormat<ITuple, NullWritable> implements Serializable {
	...
	/**
	 * Character separated files reader. You must specify the Schema that will be used for Tuples being read so that
	 * automatic type conversions can be applied (i.e. parsing) and the CSV semantics (if any). Use
	 * {@link #NO_ESCAPE_CHARACTER} and {@link #NO_QUOTE_CHARACTER} if the input files don't have any such semantics. If
	 * hasHeader is true, the first line of any file will be skipped.
	 * <p>
	 ...
	 * <li>A "null string" text which will be substituted by null if found. For example, the sequence "\N" used by MySQL
	 * dumps. The null string can be null itself which means this feature is disabled.</li>
	 * </ul>
	 */
	...
	/**
	 * Fixed width fields file reader. You must specify the Schema that will be used for Tuples being read so that
	 * automatic type conversions can be applied (i.e. parsing). If hasHeader is true, the first line of any file will be
	 * skipped.
	 * <ul>
	 * <li><strong>fields</strong>: must contains pairs of positions, indicating the location of fields in the line. The
	 * array must contains pairs of starPosition and endPosition, so that the substring including the characters at
	 * startPosition and endPosition is a field. The first characters is at position 0 of the line.</li>
	 * <li><strong>nullString</strong>: considering a field null when found. No effect if null. The field is trimmed
	 * before performed the comparison.</li>
	 * </ul>
	 */
	...
	private void checkFixedWithFields(Schema schema, int[] fields) {
		if(schema.getFields().size() * 2 != fields.length) {
			throw new IllegalArgumentException(
			    "Array with field positions for fixed width fields of incorrect size [" + fields.length
			        + "]. Expected size [" + schema.getFields().size() * 2 + "] for the schema [" + schema
			        + "] of size [" + schema.getFields().size() + "]");
		}
		for(int i = 0; i < fields.length; i += 2) {
			int min = fields[i];
			int max = fields[i + 1];
			if(max < min) {
				throw new IllegalArgumentException("Incorrect field range [" + min + "," + max
				    + "]. max position cannot be smaller than min position.");
			}
		}
	}
		...
		public boolean nextKeyValue() throws IOException {
			...
			while(position < end) {
				...
				if(newSize < maxLineLength && newSize > 0) {
					...
					try {
						readLine = tokenizer.tokenizeLine(line.toString(), csvStrategy, null);
					} catch(Throwable e) {
						LOG.warn("Error parsing line: (" + line + "). Broken encoding?");
						LOG.debug(e);
					}
					...
					for(int i = 0; i < schema.getFields().size(); i++) {
						...
						try {
							...
							LOG.warn("Error parsing value: (" + currentValue + ") in text line: (" + readLine
	...
	public RecordReader<ITuple, NullWritable> createRecordReader(InputSplit iS, TaskAttemptContext context)
		...
		if(type == InputType.CSV) {
			return new TupleTextInputReader(schema, hasHeader, strictQuotes, separatorCharacter,
			    quoteCharacter, escapeCharacter, fieldSelector, nullString);
		} else {
