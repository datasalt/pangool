 ...
 * 
 * student_scores:
 * (student,score)  PK(student)
 * 
 * student_info
 * (student,age,country)  PK(student)
 * 
 * This example calculates the average,variance and standard deviation of the scores for every
 * country, only for students older than 15 years old.
 *
 ...
 *  student_scores
 *  student_info
 * Task : filter only the students older than 15 years old, join the two datasets grouping by "country"
 * Output : (country,score)
 * 
 * 2nd job:
 * Inputs : 
 * (country,score) <= output from 1st job
 * Task: just compute the average for every country, grouping by country
 * Output : (country,average)
 * 
 * 3rd job:
 * Inputs:
 * - (country,average) <= output from 2nd job
 * - (country,score) <= output from 1st job 
 * Task : calculate the variance, and deviation
 * Output : (country,average,variance,stdev)
 * 
 */
...
public class StandardDeviationFlow extends LinearFlow {
	...
	public StandardDeviationFlow(String studentsFile, String scoresFile, String output,int minAge)
		...
		final Schema finalOutSchema = new Schema("finalOutSchema", Fields.parse("country:string,average:double,variance:double,stdev:double"));
		...
		add(new FlowMR("job1", new Inputs("students","scores"), new Params(new Param("minAge",Integer.class)),
			...
			public void configure(Map<String, Object> parameters) throws TupleMRException {
				...
				// Define the Reducer
				setReducer(	new SingleSchemaReducer<ITuple,NullWritable>(countryScoresSchema) {
		...
		add(new FlowMR("job2", new Inputs("input"), Params.NONE,
				NamedOutputs.NONE, new GroupBy("country"), null) {
			...
			public void configure(Map<String, Object> parameters) throws TupleMRException {
				...
				setReducer(	new SingleSchemaReducer<ITuple,NullWritable>(countryAveragesSchema) {
		...
		add(new FlowMR("job3", new Inputs("country_averages","country_scores"), 
				Params.NONE, NamedOutputs.NONE,new GroupBy("country"),new OrderBy().add("country",Order.ASC).addSchemaOrder(Order.ASC)) {
			...
			public void configure(Map<String, Object> parameters) throws TupleMRException {
				...
				setReducer(new SingleSchemaReducer<ITuple, NullWritable>(finalOutSchema) {
